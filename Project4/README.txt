...
NOTE: The process takes one more argument that it will simply ignore becuase we did not implement a cache.
...
The dispatchers and receivers essentially worker as a bounded buffer.
Upon receiving a request, a dispatcher thread will acquire the access lock, prepare that request, and check if the queue is already full. If so, it will wait until it receives a condition signal. It then puts the appropriate information in the queue, releases the access lock and signals the workers for a new request, then loops around.
